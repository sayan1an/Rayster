...
class RtxFiltering_1 : public WindowApplication 
{
private:
...
public:
		...
		const RandomSquarePattern& randomPattern)
	{
		...
		uint32_t missShaderId0 = rtxPipeGen.addMissShaderStage(device, ROOT + "/shaders/RtxFiltering_1/01_miss.spv");
		uint32_t missShaderId1 = rtxPipeGen.addMissShaderStage(device, ROOT + "/shaders/RtxFiltering_1/02_miss.spv");
		uint32_t hitGroupId0 = rtxPipeGen.startHitGroup();
		rtxPipeGen.endHitGroup();
		uint32_t hitGroupId1 = rtxPipeGen.startHitGroup();
		rtxPipeGen.addCloseHitShaderStage(device, ROOT + "/shaders/RtxFiltering_1/02_close.spv");
		rtxPipeGen.endHitGroup();
...
public:
	...
	void createSubpass(const VkDevice& device, const VkExtent2D& swapChainExtent, const VkRenderPass& renderPass, FboManager& fboMgr)
	{
		...
		gfxPipeGen.addVertexShaderStage(device, ROOT + "/shaders/RtxFiltering_1/gShowVert.spv");
		gfxPipeGen.addFragmentShaderStage(device, ROOT + "/shaders/RtxFiltering_1/gShowFrag.spv");
...
public:
	RtxFiltering_1(const std::vector<const char*>& _instanceExtensions, const std::vector<const char*>& _deviceExtensions, const std::vector<const char*>& _deviceFeatures) :
