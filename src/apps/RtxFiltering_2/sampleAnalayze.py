...
def plotHistogram(rawData):
    ...
    plt.hist2d(x=samples[:index2, 0], y=samples[:index2, 1], bins=[edges, edges], weights=samples[:index2, 2], density=True)
    plt.show()
...
def computePerFrameMeanVar(rawData, refMean, refVar_x, refVar_y, refVar_xy):
    errList = []
    ...
    while (index < rawData.shape[0]):
        ...
        movingWeight = movingWeight + weight
