...
namespace RtxFiltering_3
{
	...
	public:
		void createPipeline(const VkPhysicalDevice& physicalDevice, const VkDevice& device, const VkImageView& inNormal, const VkImageView &inPrevFrame)
		{
			...
			filterPipeGen.addComputeShaderStage(device, ROOT + "/shaders/RtxFiltering_3/stencilPass.spv");
	...
	public:
		void createPipeline(const VkPhysicalDevice& physicalDevice, const VkDevice& device, const VkImageView& stencilView, const VkImageView& stencilView2, const VkImageView& stencilView3)
		{
			...
			filterPipeGen.addPushConstantRange({ VK_SHADER_STAGE_COMPUTE_BIT , 0, sizeof(PushConstantBlock) });
			filterPipeGen.addComputeShaderStage(device, ROOT + "/shaders/RtxFiltering_3/stencilCompositionPass.spv");
			filterPipeGen.createPipeline(device, descriptorSetLayout, &pipeline, &pipelineLayout);
		}
