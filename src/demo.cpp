...
private:
	...
	void initVulkan() {
		...
		// Framebuffer blueprint was changed in createRenderPass(), hence attach the color, depth and colorResolve attachment accordingly.
		createFramebuffers();
		createTextureImage();
	...
	void createRenderPass() {
		...
		// ref to swap chain image buffer
		...
		subpass.pColorAttachments = &colorAttachmentRef; // changed
		subpass.pDepthStencilAttachment = &depthAttachmentRef;
		subpass.pResolveAttachments = &colorAttachmentResolveRef; //changed
		...
		std::array<VkAttachmentDescription, 3> attachments = { colorAttachment, depthAttachment, colorAttachmentResolve }; // changed
	...
	void createGraphicsPipeline() {
...

		{
			...
			VkGraphicsPipelineCreateInfo pipelineInfo = {};
			subpass2.createDefaultGraphicsPipelineInfo(device, ROOT + "/shaders/02_vert.spv", ROOT + "/shaders/02_frag.spv",
			...
			subpass2.createRasterizationStateInfo(rasterizer);
			rasterizer.cullMode = VK_CULL_MODE_NONE;

			VkPipelineDepthStencilStateCreateInfo depthStencil = {};
			subpass2.createDepthStencilStateInfo(depthStencil);
			...
			if (vkCreateGraphicsPipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &subpass2.pipeline) != VK_SUCCESS) {
				throw std::runtime_error("failed to create graphics pipeline!");
			}
